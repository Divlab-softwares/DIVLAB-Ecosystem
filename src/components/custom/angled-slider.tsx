...
const AngledCard = ({
    ...
    return (
        <motion.a
            ...
            href={`/course_details/courseId=${item.id}`} // Assuming you want to link to course details
        ...
        >
            ...
            min-h-[300px] shadow-2xl rounded-lg">
                <Image
                    fill
                    src={imageSrc}
                    alt={item.domain || "Slider Image"}
                ...
                {item.title && (
                    <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/80 to-transparent p-4 text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                        <h3 className="text-lg font-bold">{item.title}</h3>
                        <span className="text-sm font-normal ">{item.domain}</span>
                    </div>
                )}

                <div className="absolute h-60 bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    {/* <h3 className="text-lg font-bold">{item.title}</h3>
                    <span className="text-sm font-normal ">{item.domain}</span> */}
                </div>
            </div>
