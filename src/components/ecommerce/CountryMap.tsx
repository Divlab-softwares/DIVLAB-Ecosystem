...
import { getCountryLatLng } from "@@/lib/countryMapper";
...
const CountryMap: React.FC<CountryMapProps> = ({ mapColor, locations }) => {

  
  const markers = locations
    .map((loc) => {
      const coords = getCountryLatLng(loc.location);

      if (!coords) return null;

      return {
        latLng: coords,
        name: `${loc.location} (${loc.count})`,
        style: {
          fill: "#465FFF",
          borderWidth: 1,
          borderColor: "white",
